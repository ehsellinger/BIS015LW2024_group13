...
2.  Read in data:
```{r}
conservatory <- read_csv("Elisabeth/conservatory_data.csv") %>% clean_names()
conservation_status <- read_csv("Elyse/Biodiversity_Data.csv")
```
...
ui <- fluidPage(    
  ...
  # define the sidebar with one input
  sidebarPanel(
    selectInput("x", "Select Family", choices = unique(conservatory_families$region), hr())
),  
