...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      selectInput("x", "Select Plant Family", choices = unique(data3$family)),
...
server <- function(input, output, session) {
  ...
  output$region_selector <- renderUI({
    req(input$x)
    regions <- unique(data3$region[data3$family == input$x])
  ...
  output$plot <- renderPlot({
    ...
    if (input$region == "All") {
      ...
      filtered_data <- data3[data3$family == input$x & data3$region == input$region, ]
    }
    ggplot(filtered_data, aes(x = genus, fill = endangered_status)) +
      geom_bar() +
      labs(x = "Genus",
           y = "Count")
  })
...
server <- function(input, output, session) {
  ...
  output$region_selector <- renderUI({
    req(input$x)
    regions <- c("All", unique(data3$region[data3$family == input$x]))
  ...
  observeEvent(input$region, {
    ...
    if (!is.null(input$region)) {
      ...
      plot_list <- lapply(regions, function(region) {
        filtered_data <- if (region == "All") {
          data3[data3$family == input$x, ]
        } else {
          data3[data3$family == input$x & data3$region == region, ]
        }
        
        plotOutput(paste0("plot_", region), height = "300px")
      })
  ...
  observeEvent(input$region, {
    ...
    regions <- if (input$region == "All") {
      unique(data3$region[data3$family == input$x])
    } else {
      input$region
    }
    
    for (region in regions) {
      output[[paste0("plot_", region)]] <- renderPlot({
        filtered_data <- if (input$region == "All") {
          data3[data3$family == input$x, ]
        } else {
          data3[data3$family == input$x & data3$region == region, ]
        }
        
        ggplot(filtered_data, aes(x = genus)) +
          geom_bar() +
          labs(x = "Genus",
               y = "Count") +
